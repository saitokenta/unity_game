...
my $config = +{
    controller  => {
        dirs  => [qw/lib Game2D Controller/],
    },
    schema      => {
        dirs => [qw/lib Game2D Schema Result/],
    },
    view        => {
        dirs => [qw/lib Game2D View/],
    },
    module      => {
        dirs => [qw/lib/],
    },
...
@@ controller.mt
package Game2D::Controller::<?= $name ?>;
use Ark 'Controller';
...
@@ schema.mt
package Game2D::Schema::Result::<?= $name ?>;

use strict;
use warnings;
use utf8;
use parent qw/Game2D::Schema::ResultBase/;

use Game2D::Schema::Types;
use Game2D::Models;
